...
describe Aypex::Order, type: :model do
  ...
  context "#finalize!" do
    ...
    it "sends an order confirmation email to customer" do
      ...
      expect(Aypex::OrderMailer).to receive(:confirm_email).with(order.id).and_return mail_message
    ...
    it "does not send duplicate confirmation emails" do
      ...
      expect(Aypex::OrderMailer).not_to receive(:confirm_email)
      order.finalize!
    end
    ...
    context "new order notifications" do
      it "sends a new order notification email to store owner when notification email address is set" do
        mail_message = double "Mail::Message"
        expect(Aypex::OrderMailer).to receive(:store_owner_notification_email).with(order.id).and_return mail_message
        expect(mail_message).to receive :deliver_later
        order.finalize!
      end
      ...
      it "does not send a new order notification email to store owner when notification email address is blank" do
        ...
        expect(Aypex::OrderMailer).to_not receive(:store_owner_notification_email)
        order.finalize!
      end
    end
  end
  ...
  context "#cancel" do
    ...
    before do
      ...
      allow_any_instance_of(Aypex::OrderUpdater).to receive(:update_adjustment_total).and_return(10)
    end
    ...
    it "sends a cancel email" do
      ...
      mail_message = double "Mail::Message"
      order_id = nil
      expect(Aypex::OrderMailer).to receive(:cancel_email) { |*args|
        order_id = args[0]
        mail_message
      }
      expect(mail_message).to receive :deliver_later
