...
describe Aypex::ReimbursementMailer, type: :mailer do
  ...
  context ":from not set explicitly" do
    it "uses store mail from address" do
      message = Aypex::ReimbursementMailer.reimbursement_email(reimbursement)
  ...
  context ":reply_to not set explicitly" do
    it "uses store mail from address" do
      message = Aypex::ReimbursementMailer.reimbursement_email(reimbursement)
      expect(message.reply_to).to eq [Aypex::Store.default.mail_from_address]
    end
  end

  it "accepts a reimbursement id as an alternative to a Reimbursement object" do
    expect(Aypex::Reimbursement).to receive(:find).with(reimbursement.id).and_return(reimbursement)

    expect do
      Aypex::ReimbursementMailer.reimbursement_email(reimbursement.id).body
    ...
    context "reimbursement_email" do
      context "pt-BR locale" do
        before do
          ...
          pt_br_shipped_email = {aypex: {reimbursement_mailer: {reimbursement_email: {dear_customer: "Caro Cliente,"}}}}
          I18n.backend.store_translations :"pt-BR", pt_br_shipped_email
          Aypex::Store.default.update(default_locale: "pt-BR")
        end
        ...
        it "localized in HTML template" do
          reimbursement_email = Aypex::ReimbursementMailer.reimbursement_email(reimbursement)
