...
describe Aypex::OrderMailer, type: :mailer do
  ...
  it "confirm_email accepts an order id as an alternative to an Order object" do
    expect(Aypex::Order).to receive(:find).with(order.id).and_return(order)
    expect do
  ...
  context "displays unit costs from line item" do
    ...
    # Tests mailer view aypex/order_mailer/confirm_email.text.erb
    specify do
      confirmation_email = described_class.confirm_email(order)
    ...
    # Tests mailer view aypex/order_mailer/cancel_email.text.erb
    specify do
  ...
  context "emails must be translatable" do
    context "pt-BR locale" do
      before do
        ...
        pt_br_confirm_mail = {aypex: {order_mailer: {confirm_email: {dear_customer: "Caro Cliente,"}}}}
        pt_br_cancel_mail = {aypex: {order_mailer: {cancel_email: {order_summary_canceled: "Resumo da Pedido [CANCELADA]"}}}}
