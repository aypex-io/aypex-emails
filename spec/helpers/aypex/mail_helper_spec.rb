...
describe Aypex::MailHelper, type: :helper do
  ...
  describe "#name_for" do
    ...
    context "without address" do
      it "shows default name" do
        expect(subject).to eq Aypex.t("customer")
      end
    end
  ...
  describe "#logo_path" do
    ...
    context "when @order exists" do
      let(:logo_image) { File.open(File.expand_path("../../fixtures/files/img_256x128.png", __dir__)) }

      before do
        store.build_mailer_logo
        store.mailer_logo.attachment.attach(io: logo_image, filename: "img_256x128.png", content_type: "image/png")
